...
# === Leer GeoJSON en lugar del CSV ===
geojson_path = root_path / "data" / "pois_validados_flexible.geojson"
...
for feature in geojson_input["features"]:
    ...
    props["POI_LON"] = coords[0]
    props["POI_LAT"] = coords[1]
    data.append(props)
...
for index, row in df.iterrows():
    ...
    if get_satellite_tile(lat, lon, ZOOM_LEVEL, TILE_FORMAT, API_KEY, poi_id):
        ...
        results = model(img_path, imgsz=1536, conf=0.05, verbose=False)[0]
...
# === Clasificación de observaciones y tipos de error ===
def asignar_observacion(row):
    if row["Contiene_estructura"]:
        return "estructura"
    elif row["Contiene_árbol"]:
        return "arbol"
    elif row["Contiene_agua"]:
        return "agua"
    elif row["Contiene_tren"]:
        return "vias"
    else:
        return "ninguna"

def asignar_tipo_error(row):
    if row["Contiene_estructura"]:
        return "Errror tipo 4"
    elif row["Contiene_agua"] or row["Contiene_tren"] or row["Contiene_árbol"]:
        return "Error tipo 3"
    else:
        return "Error tipo 1"

df_det["Observacion"] = df_det.apply(asignar_observacion, axis=1)
df_det["Tipo_error"] = df_det.apply(asignar_tipo_error, axis=1)

# ✅ Solo guardar las tres columnas necesarias
# df_det[["POI_ID", "Observacion", "Tipo_error"]].to_csv("Clasificacion_de_sospechosos.csv", index=False)
# print("✅ Archivo Clasificacion_de_sospechosos.csv generado con solo POI_ID, Observacion y Tipo_error.")

# # === Guardar CSV filtrado: excluir estructura ===
# df_filtrado = df_det[df_det["Observacion"] != "estructura"]
# df_filtrado[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_sin_estructura.csv", index=False)
# print("✅ Archivo POIs_sin_estructura.csv generado (sin observaciones de tipo 'estructura').")

# # === Guardar CSV filtrado: excluir estructura ===
# df_estructura = df_det[df_det["Observacion"] == "estructura"]
# df_estructura[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_solo_estructura.csv", index=False)
# print("✅ Archivo POIs_solo_estructura.csv generado (sin observaciones de tipo 'estructura').")

# # === Guardar CSV filtrado: agua, tren o arbol ===
# df_agua_arbol_vias = df_det[(df_det["Observacion"] == "arbol") | 
#                             (df_det["Observacion"] == "agua") | 
#                             (df_det["Observacion"] == "vias")]
# df_agua_arbol_vias[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_solo_agua_arbol_vias.csv", index=False)
# print("✅ Archivo POIs_solo_agua_arbol_vias.csv generado (sin observaciones de tipo 'estructura').")

# # === Guardar CSV filtrado: ninguna ===
# df_ninguna = df_det[df_det["Observacion"] == "ninguna"]
# df_ninguna[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_sin_ninguna.csv", index=False)
# print("✅ Archivo POIs_sin_ninguna.csv generado (sin observaciones de tipo 'estructura').")

# # === Guardar las cosas como tipo geojson ===

def guardar_geojson(df, nombre_archivo):
    features = []
...
# === Guardar CSV general
df_det[["POI_ID", "Observacion", "Tipo_error"]].to_csv("Clasificacion_de_sospechosos.csv", index=False)
guardar_geojson(df_det, "Clasificacion_de_sospechosos.geojson")

# === Sin estructura
df_filtrado = df_det[df_det["Observacion"] != "estructura"]
df_filtrado[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_sin_estructura.csv", index=False)
guardar_geojson(df_filtrado, "POIs_sin_estructura.geojson")

# === Solo estructura
df_estructura = df_det[df_det["Observacion"] == "estructura"]
df_estructura[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_solo_estructura.csv", index=False)
guardar_geojson(df_estructura, "POIs_solo_estructura.geojson")

# === Agua, árbol, vías
df_agua_arbol_vias = df_det[df_det["Observacion"].isin(["arbol", "agua", "vias"])]
df_agua_arbol_vias[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_solo_agua_arbol_vias.csv", index=False)
guardar_geojson(df_agua_arbol_vias, "POIs_solo_agua_arbol_vias.geojson")

# === Ninguna
df_ninguna = df_det[df_det["Observacion"] == "ninguna"]
df_ninguna[["POI_ID", "Observacion", "Tipo_error"]].to_csv("POIs_sin_ninguna.csv", index=False)
guardar_geojson(df_ninguna, "POIs_sin_ninguna.geojson")

print("✅ Todos los archivos CSV y GeoJSON fueron generados correctamente.")

# === Guardar nuevo pois.geojson con observación y tipo de error
features = []
...
print("✅ pois.geojson actualizado con observaciones y tipo de error.")
